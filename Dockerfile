...
RUN gunzip -c modsecurity-v${MODSECURITY_VERSION}.tar.gz | tar xvf -
RUN mv modsecurity-v${MODSECURITY_VERSION} ModSecurity
...
RUN gunzip -c nginx-${NGINX_VERSION}.tar.gz | tar xvf -
...
RUN gunzip -c modsecurity-nginx-v${MODSECURITY_NGINX_VERSION}.tar.gz | tar xvf -
...
RUN wget https://github.com/SpiderLabs/owasp-modsecurity-crs/archive/v${RULESET_VERSION}.tar.gz
RUN tar -xzf v${RULESET_VERSION}.tar.gz -C /etc/nginx
RUN rm v${RULESET_VERSION}.tar.gz

# Install ModSecurity CRS
RUN cat /etc/nginx/owasp-modsecurity-crs-${RULESET_VERSION}/crs-setup.conf.example /etc/nginx/owasp-modsecurity-crs-${RULESET_VERSION}/rules/*.conf >> /etc/nginx/modsecurity.d/crs.conf
RUN cp /etc/nginx/owasp-modsecurity-crs-${RULESET_VERSION}/rules/*.data /etc/nginx/modsecurity.d/
